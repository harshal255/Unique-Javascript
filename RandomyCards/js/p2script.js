...
function play1() {
    ...
    if (middleDeck.length != 0 && card[0] === middleDeck[middleDeck.length - 1][0]) {
        ...
        alert("Its a match for player 1");
...
function play2() {
    ...
    if (middleDeck.length != 0 && card[0] === middleDeck[middleDeck.length - 1][0]) {
        ...
        shuffle(P2Cards);
        middleDeck = [];
        alert("Its a match for player 2");
        imageHolder1.innerHTML="";
        imageHolder2.innerHTML="";
    } else {
        //adding image in middle row
...
shuffle1.addEventListener('click',()=>{
    for (let i = P1Cards.length - 1; i >= 0; i--) {
        // console.log(i);
        let randomIndex = Math.floor(Math.random() * i);
        // console.log('random: ',randomIndex);
        let temp = P1Cards[randomIndex];
        P1Cards[randomIndex] = P1Cards[i];
        P1Cards[i] = temp;
    }
})
