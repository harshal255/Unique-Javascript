...
function displayProjects() {
  ...
  if (keyword === "") {
    ...
    let filtered_projects = Projects.filter(function (project) {
      return project.topic.toLowerCase().includes(keyword);
    });

    appendProjects(filtered_projects);
  }
}
...
function updatePageButtons(array) {
  // Clear existing page buttons
  pageButtonsContainer.innerHTML = "";

  // Calculate the total number of pages
  const totalPages = Math.ceil(array.length / projectsPerPage);
